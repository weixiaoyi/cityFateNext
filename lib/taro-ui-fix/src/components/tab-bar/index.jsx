...
export default class AtTabBar extends AtComponent {
    ...
    render() {
        ...
        {tabList.map((item, i) => (<View className={classNames('at-tab-bar__item', {
            ...
            {item.iconType ? (<AtBadge dot={!!item.dot} value={item.text} maxValue={Number(item.max)}>
            ...
            {item.image ? (<AtBadge dot={!!item.dot} value={item.text} maxValue={Number(item.max)}>
                <View className='at-tab-bar__icon'>
            ...
            <View>
              <AtBadge dot={item.iconType || item.image ? false : !!item.dot} value={item.iconType || item.image ? '' : item.text} maxValue={item.iconType || item.image ? 0 : Number(item.max)}>
